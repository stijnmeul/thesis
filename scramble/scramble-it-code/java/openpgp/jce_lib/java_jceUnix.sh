...
then
	...
	then
		...
		if [ $java_home != "${string/bin/}" ]; then
			echo "JAVA_HOME directory points to bin/ aiming folder one layer bellow"
	...
	# Backup old files
	echo $file1" | "$file2
	echo $pwd | sudo -S cp $full_path$file1".jar" $full_path$file1"2.jar"
	echo "File "$file1" backup complete"
	sudo cp $full_path$file2".jar" $full_path$file2"2.jar"
