    ...
    \scalebox{0.78}{
        \begin{tikzpicture}[auto, node distance=-2mm, align=center,
            ...
            \path
                ...
                (0,5) node [leg,fill=white] (white_block) {Entities with access to $A$'s Profile $\mathcal{V}_A$}
                (6,4) node [color=cyan,leg,fill=white] (white_block) {Intended Recipient Set $\mathcal{S}$}
                (6,4.5) node [leg,fill=white] (white_block) {Friends Set $\mathcal{F}_A$}
...
\paragraph{No redundancy} The broadcast message $\mathcal{B}$ should only be published once since $\mathcal{B}$ contains a concatenation of $w_i$ describing the with IBE encrypted session key $k$ for every recipient $i$ in $\mathcal{S}$.
